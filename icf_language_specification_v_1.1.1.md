...
**Errata policy:** corrections to stable versions are published as errata or patch releases at <https://icformat.org/versions/>; published texts are never silently edited.
...
| 1.1.1   | 2026-08-21 | Clarification patch: adds Scope (§80), Security Considerations (§81), Implementation Limits (§82) and Conformance (§83). No change to document validity or interpretation; all 1.1 section numbers are unchanged. |
...
* the ICF **syntax** (directives, schema declarations, masters, records, rows, annotations, text blocks, comments and escaping);
* the ICF **abstract data model** (Appendix B) and how conforming text maps onto it;
* the **interpretation and processing model**, including resolution of defaults, overrides, primary objects and master references;
* the **validation rules**, including the diagnostic conditions a processor is required to detect;
* the **canonical content** used for checksums (§67–§68).

This specification does **not** define:

* transport, storage, compression or encryption of ICF documents;
* authentication or authorization semantics (checksums provide integrity, not authenticity);
* application-specific schemas or the meaning of application data;
* a query language;
* the companion index format **ICX**, which is specified separately and is optional and derived — an ICF document is complete without it.

---

# 81. Security Considerations

ICF documents are plain-text structured data. They contain no executable or active content, and no construct in this specification requires a processor to fetch external resources. Directives or metadata values that carry URLs (including user-defined directives, §10) are informative: processors SHOULD NOT automatically dereference them when handling untrusted input.

Processors handling untrusted documents SHOULD treat the following as attack surfaces and guard against resource exhaustion:

* very large documents, very long lines, and very large preformatted text blocks;
* deeply indented structures (nesting depth);
* very large numbers of records, rows, annotation entries or schema nodes;
* pathological escape sequences and delimiter-heavy rows.

ICF performs no Unicode normalization: identifiers and values are compared by code point. Processors displaying untrusted documents SHOULD apply the usual care with invisible, bidirectional-control and confusable characters.

Checksum directives (`@checksum`, and the ICX source checksums) are an **integrity** mechanism only; they SHALL NOT be treated as proof of origin. Documents that fail checksum verification SHOULD be reported, not silently accepted or repaired.

---

# 82. Implementation Limits

Implementations MAY impose resource limits — for example maximum document size, line length, nesting depth, or record count — provided the limits are documented.

Exceeding an implementation limit SHOULD produce a clear diagnostic identifying the limit, and SHALL NOT be reported as a syntax error in the document.

Implementation limits do not affect document validity: a document that exceeds one implementation's limits remains a conforming ICF document.

---

# 83. Conformance

An implementation conforms to ICF 1.1 if it satisfies the mandatory (SHALL-level) requirements of this specification and passes the applicable tests of the official conformance suite, subject to declared implementation limits (§82).

The official, versioned conformance suite is published at:

* <https://github.com/icformat/icf-conformance>

The suite verifies diagnostics (severity and code), the parsed data model, and write round-trips against fixture documents with stable test identifiers. Diagnostic message wording and line numbers are implementation-defined; diagnostic **codes and severities** are normative and are listed in the suite manifest.

Where a test expectation and this specification appear to disagree, this specification is authoritative and the discrepancy should be reported as a suite defect.
